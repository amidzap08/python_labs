...
#pyproject.toml — это единый файл Python-проекта для зависимостей;инструментов: black, pytest и т.д.;
#Формат TOML — читаемый, похож на ini, но современнее. 
#pytest — это набор правил для написания и запуска тестов в Python.
#Параметризация — это способ запустить один и тот же тест много раз с разными входными данными и ожидаемым результатом.assert
#Фикстура — это функция, которую pytest вызывает до теста, чтобы подготовитьcя создать временный файл или директорию;
